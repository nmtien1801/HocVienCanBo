...
export default function LookupGraduationExam() {
    ...
    const fetchListSearchPoint = async (customPage = currentPage, customLimit = pageSize) => {
        ...
        setError(null);
        try {
            let res = await dispatch(getSearchPointGraduation({ studentCode: studentCode, subjectID: selectedSubject, page: customPage, limit: customLimit }));
            if (!res.payload || !res.payload.data) {
    ...
    const handleExportExcel = async () => {
        ...
        try {
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, "danh sách điểm thi tốt nghiệp");
            XLSX.writeFile(workbook, `Danh_sach_diem_thi_tot_nghiep_${new Date().getTime()}.xlsx`);
