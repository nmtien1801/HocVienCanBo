...
export default function FinalExam1() {
  ...
  const { ExaminationList, ExaminationTotal } = useSelector((state) => state.point);
  ...
  const fetchListExamination = async (customPage = currentPage, customLimit = pageSize) => {
    ...
    setError(null);
    try {
      let res = await dispatch(getListExamination({ classID: selectedClass, subjectID: selectedSubject, page: customPage, limit: customLimit }));
  ...
  const handleExportExcel = async () => {
    if (!ExaminationTotal || ExaminationTotal === 0) {
    ...
    try {
      ...
      const fullData = await fetchListExamination(1, ExaminationTotal);
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "danh sách dự thi cuối môn lần 1");
      XLSX.writeFile(workbook, `Danh_sach_du_thi_cuoi_mon_lan_1_${new Date().getTime()}.xlsx`);
  ...
  const totalPages = Math.ceil(ExaminationTotal / pageSize);
  ...
  const renderTableBody = () => {
    ...
    // Data Rows
    return ExaminationList.map((row, index) => (
      <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
        ...
        <td className="px-4 py-3 border-r border-gray-200 text-center">{row.PHACH}</td>
        <td className="px-4 py-3 border-r border-gray-200 text-center">{row.RoomName}</td>
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-[1600px] mx-auto">
        {/* Header */}
        <h1 className="text-xl md:text-2xl text-gray-600 mb-6">Danh sách dự thi cuối môn lần 1</h1>
          ...
          <div className="flex flex-col md:flex-row flex-wrap items-stretch md:items-center gap-4 md:gap-6">
            ...
            <div className='flex gap-4'>
              ...
              <button
                ...
                disabled={isLoading || !ExaminationList || ExaminationList.length === 0}
              >
                <FileDown size={16} />
        ...
        <div className="bg-white rounded-lg shadow-sm">
          ...
          {/* Pagination */}
          {!isLoading && !error && ExaminationList && ExaminationList.length > 0 && (
            ...
            <div className="p-4 md:px-6 md:py-4 border-t border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-end gap-4">
                <span className="text-sm text-gray-600 whitespace-nowrap">
                  Hiển thị {((currentPage - 1) * pageSize) + 1} - {Math.min(currentPage * pageSize, ExaminationTotal)} / {ExaminationTotal} kết quả
                </span>
                <div className="flex items-center gap-2">
